...
from net import ssd_mobile_net
...
# Configure this point to change the backbone network
tf.app.flags.DEFINE_string(
    'checkpoint_path', './model/mobilev2',
    'The path to a checkpoint from which to fine-tune.')
tf.app.flags.DEFINE_string(
    'checkpoint_model_scope', '',
    'Model scope in the checkpoint. None if the same as the trained model.')
tf.app.flags.DEFINE_string(
    'model_scope', 'ssd_mobile',
    'Model scope name used to replace the name_scope in checkpoint.')
tf.app.flags.DEFINE_string(
    'checkpoint_exclude_scopes', 'ssd_mobile/multibox_head, ssd_mobile/additional_layers, ssd_mobile/conv4_3_scale',
...
def input_pipeline(dataset_pattern='train-*', is_training=True, batch_size=FLAGS.batch_size):
    def input_fn():
        ...
        anchor_creator = anchor_manipulator.AnchorCreator(out_shape,
                                                    layers_shapes = [(56, 56), (28, 28), (14, 14), (7, 7)],
                                                    anchor_scales = [(0.15,), (0.25,), (0.55,), (0.725,)],
                                                    extra_anchor_scales = [(0.1414,),  (0.4541,), (0.8078,), (0.9836,)],
                                                    anchor_ratios = [(1., 2., .5), (1., 2., 3., .5, 0.3333), (1., 2., 3., .5, 0.3333), (1., 2., .5)],
                                                    layer_steps = [8, 32, 100, 300])
        ...
        anchor_encoder_decoder = anchor_manipulator.AnchorEncoder(allowed_borders = [1.0] * 4,
...
def ssd_model_fn(features, labels, mode, params):
    """model_fn for SSD to be used with our Estimator.
        ...
        inputs:
            - features: Tensor shape=(batch_size, 3, 300(height), 300(width))
            - labels: a dict {'shape': <tf.Tensor 'batch:2' shape=(32, 3) dtype=int64>, 
                'cls_targets': <tf.Tensor 'batch:4' shape=(32, 8732) dtype=int64>, 
                'loc_targets': <tf.Tensor 'batch:3' shape=(32, 8732, 4) dtype=float32>, 
                'match_scores': <tf.Tensor 'batch:5' shape=(32, 8732) dtype=float32>}
    ...
    with tf.variable_scope(params['model_scope'], default_name=None, values=[features], reuse=tf.AUTO_REUSE):
        ...
        backbone = ssd_mobile_net.MobileNetv2SSD(features, params['data_format'])
        feature_layers = backbone.get_feature_layers()

        # feature_layers = backbone.forward(features, training=(mode == tf.estimator.ModeKeys.TRAIN))
        # get feature layers
        location_pred, cls_pred = ssd_mobile_net.multibox_head(feature_layers, params['num_classes'], all_num_anchors_depth, data_format=params['data_format'])
